...
class Clientlet(Thread):
    ...
    def unclaim(self, source_context, filename):
        ...
        self.break_claim(source_context, filename)
    ...
    def renew_claim(self, source_context, filename):
        rescan = self.get_interval("rescan")//2
        key = f"{source_context}:{filename}"
        self.claims[key] = time.time()


    def break_claim(self, source_context, filename):
        key = f"{source_context}:{filename}"
    ...
    def claim(self, source_context, filename, **kwargs):
        ...
        if response in ("ack", "keep"):
            self.renew_claim(source_context, filename)
...
#####  #    #  ####  #
    ...
    def claim_some(self, source_context, claims):
        response = self.send(source_context, "multiclaim", claims)
        if response:
            responses = response.value().copy() # 'response' datagram may change
            for filename, result in responses.items():
                self.logger.debug(f"multiclaim: {filename} : {result}")
                if result in ("ack", "keep"):
                    self.renew_claim(source_context, filename)
    ...
    def claim_many(self, source_context, claims):
        n = 0
        some = {}
        for filename, checksum in claims.items():
            some[filename] = checksum
            n += 1
            if n % 100 == 0:
                self.claim_some(source_context, some)


    def unclaim_many(self, source_context, unclaims):
        response = self.send(source_context, "multiunclaim", unclaims)
        # basically ignore the results
        # TODO: don't ignore the results
        for filename in unclaims:
            self.break_claim(source_context, filename)
    

    def multiclaim(self, source_context, filenames):
        claims = {}
        unclaims = []
        for filename in filenames:
            if filename in self.scanners[source_context]:
                if not self.claim_is_valid(source_context, filename):
                    filestate = self.scanners[source_context][filename]
                    claims[filename] = filestate["checksum"]
            else:
                unclaims.append(filename)
        self.claim_many(source_context, claims)
        self.unclaim_many(source_context, unclaims)
    ...
    def reinventory_source(self, source_context):
        ...
        self.multiclaim(source_context, inventory)
        ...
        self.multiclaim(source_context, orphans)


    def reinventory(self):
        for source_context in self.random_source_list:
            if self.send(source_context, "heartbeep"):
                self.reinventory_source(source_context)
