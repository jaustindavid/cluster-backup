...
#                 self.clients[filename].append(client)
    ...
    def multiclaim(self, args):
        # self.logger.debug(f"starting multiclaims: {args}")
        client, claims = args[:2]
        # self.logger.debug(f"claims: {type(claims)} is {claims}")
        response = {}
        for filename, checksum in claims.items():
            response[filename] = self.claim((client, filename, checksum))
        # self.logger.debug(f"multiclaim responds {response}")
        return response
    ...
    def multiunclaim(self, args):
        client, unclaims = args[:2]
        response = {}
        for filename in unclaims:
            response[filename] = self.unclaim((client, filename))
        self.logger.debug(f"multiunclaim responds {response}")
        return response
    ...
    def handle(self, action, args):
        ...
        actions = {"request":       self.request,
                    "claim":        self.claim,
                    "multiclaim":   self.multiclaim,
                    "unclaim":      self.unclaim,
                    "multiunclaim": self.multiunclaim,
                    "drain":        self.drain,
